...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "hyperspectral_camera_in");
	ros::NodeHandle nh;


    GDALDataset  *poDataset;
    GDALAllRegister();
    poDataset = (GDALDataset *) GDALOpen( "/home/conor/Downloads/ECODSEdataset/ECODSEdataset/RSdata/hs/OSBS_001_hyper.tif", GA_ReadOnly );
    if( poDataset == NULL )
    {
        //...;
    }
...
for (int raster_band_id=1; raster_band_id<poDataset->GetRasterCount()+1; raster_band_id++)
{
	...
	for (int i=0; i<nXSize; i++)
		for (int j=0; j<nYSize; j++)
		{
			...
			{
				case 290:
					raster_cloud_ptr->points[i*nYSize+j].r = pafScanline[i][j]*255/1.5;
					break;
				case 300:
					raster_cloud_ptr->points[i*nXSize+j].g = pafScanline[i][j]*255/1.5;
					break;
				case 304:
					raster_cloud_ptr->points[i*nXSize+j].b = pafScanline[i][j]*255/1.5;
					break; 
			};
		}
}
...
ros::Publisher cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("canopy_segmentation/hyperspectral_cloud", 1);
while(ros::ok())
{
